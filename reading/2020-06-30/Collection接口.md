...
集合层次的根接口，一个集合表示 一组被称为 元素的对象，

实现这个接口的 集合 可以是有序的，无序的，可以是元素重复的，可以是元素唯一的

jdk并提提供这个接口的直接 实现，而是去实现继承了这个接口的 “子接口”，

这个接口是为了 对集合有一个最大限度的抽象描述
...
无序且重复的 集合 例如 Bags 和 multisets 应该直接实现这个接口
...
一般意义上的 Collection 应该有两个 构造函数： 空参 和 只有一个参数Collection
...
一些collection允许 null 一些不允许，一些 限制指定的元素；在添加不合法元素是往往会抛出异常
NullPointerException ClassCastException ；试图 查询否一个某一个不合法元素的存在与否 会 抛出异常 或者直接
返回 false，一般，如果一个操做不会引起 非法元素的插入，那么 异常是 一个可选约束
